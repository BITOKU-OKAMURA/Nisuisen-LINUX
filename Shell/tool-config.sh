#!/bin/sh
export LFS=/mnt/lfs
cd $LFS/usr/lib64 && tar cpvf ../src/libgomp.tar ./libgomp* && /bin/rm -f ./libgomp*
cd ../../lib64 && tar xpvf ../usr/src/libgomp.tar
cd $LFS/lib64 && ln -sfv ./libgmp.so.10.0.1 ./libgmp.so.3
cd $LFS/lib64 && ln -sfv ./libmpfr.so.4.0.0 ./libmpfr.so.1
cd $LFS/lib64 && ln -sfv ../usr/lib64/libncurses.so.5 ./
cd $LFS/usr && /bin/rm -Rf ./var && ln -sfv ../var ./
cd $LFS/usr/bin && ln -sv ../../bin/install ./
cd $LFS/usr/bin && ln -sv ./ld.bfd ./ld
mkdir -p $LFS/usr/X11
...
cd $LFS/bin && ln -sv ../usr/bin/perl ./
cd /mnt/lfs/usr/lib64 && ln -sv  ../lib/libacl.so.1.1.0 libacl.so && ln -sv ./libacl.so ./libacl.so.1
cd /mnt/lfs/usr/lib64 && ln -sv  ../lib/libattr.so.1.1.0 libattr.so && ln -sv ./libattr.so ./libattr.so.1
cd /mnt/lfs/usr/lib64 && ln -sv  ../lib/libz.so.1.2.8 libz.so && ln -sv ./libz.so ./libz.so.1
...
export LC_ALL=C
PS1="[\u@\h] \W# "
alias ldconfig='ldconfig 2> /dev/null'
alias cp='/bin/cp -pvRfb'
alias ll='ls -ltra'
alias l='ll'
alias la=' ls -aF|grep "/"'
alias lla='ls -al'
alias top='nice -19 top -d 2'
alias ps='/bin/ps  -elFaw --sort -vsize|grep -v "\["|grep -v "ps -elFaw"|uniq'
EOF
...
EOF
echo "startlxde" > $LFS/root/.xinitrc 
