...
#set -h
#-------------------------------------------------------------------------------
# The Nisuisen/Linux From Scrach 0.1(Mutsuki) Installer
# 2010 12.23 Segmentation 4LT LLC
#-------------------------------------------------------------------------------
. ./config/environment
Welcome
printf "It's install program of \""
green_color "Nisuisen/Linux From Scrach"
...
while [ 1 ] ;do
    ...
    cd $LFS/usr/src/linux-2.6.35.9 && sed -i 's@-O2@-O3 -march=native@g' ./Makefile
    ...
    /bin/cp -p $JOBDIR/config/SAGYOU.3rd  $LFS/root/.2suisen/config/SAGYOU.list
    /bin/cp -p $JOBDIR/config/2suisen3rd.sh $LFS/root/.2suisen/
    echo "" >> $LFS/root/.bash_profile
    echo ". /root/.2suisen/config/environment" >> $LFS/root/.bash_profile
